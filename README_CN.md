...
- **全局风格统一**：统一输入框、选择框、文字按钮、图标按钮、分页控件的尺寸与间距，减少页面控件风格割裂。
- **列表与表格重构**：AI 提供商、认证文件、配额管理、使用统计等页面统一了表格对齐、列宽策略、分页交互与图标化操作按钮。
- **配额管理表格优化**：统一各提供商额度列表列宽策略，重点提升“周限额/次限额”可读性；文件名列支持“省略 + 点击悬浮详情”查看完整内容。
- **交互可读性提升**：长文本与模型列表统一采用省略 + 悬浮展示；成功/失败等状态颜色统一，降低信息噪音。
- **弹框与面板统一**：编辑/详情类弹框风格统一（头部、正文、底部间距与配色）；配置面板容器层级简化，信息密度更均衡。
- **配置可视化编辑优化**：恢复并规范区块顺序；`Payload` 规则区排版减层级、减少重复统计信息，支持独立展开/折叠。
- **说明文案展示优化**：将多处字段下方小字说明调整为标题后的 `?` 悬浮提示，节省纵向空间并保持视觉整洁。
- **OAuth 与认证文件体验优化**：OAuth 模型禁用/模型别名编辑页统一布局与间距，新增项交互改为更高密度且一致的可扩展行展示。
